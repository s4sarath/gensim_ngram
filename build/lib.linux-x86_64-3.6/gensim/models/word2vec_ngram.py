...
from gensim.models.utils_any2vec import _compute_ngrams_words
...
try:
    from gensim.models.word2vec_inner_ngram import  train_batch_sg_ngram , train_batch_cbow_ngram
    from gensim.models.word2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH
    logger.info("Succefully loaded Cython fast versions for word2vec")
except ImportError:
    # failed... fall back to plain numpy (20-80x slower training than the above)
    logger.info("Failed to loaded Cython fast versions for word2vec")
    FAST_VERSION = -1
    MAX_WORDS_IN_BATCH = 10000

    def train_batch_sg_ngram(model, sentences, alpha, work=None, compute_loss=False):
        """
    ...
    def train_batch_cbow_ngram(model, sentences, alpha, work=None, neu1=None, compute_loss=False):
        """
...
class Word2Vec_Ngram(BaseWordEmbeddingsModel):
    ...
    def __init__(self, sentences=None, word_gram = (1,2) , context_gram = (1,2) , keep_overlap = 1, size=100, alpha=0.025, window=5, min_count=(5,5),
                 max_vocab_size=None, sample=1e-3, seed=1, workers=3, min_alpha=0.0001,
                 sg=1, hs=0, negative=5, cbow_mean=0, hashfxn=hash, iter=5, null_word=0,
                 trim_rule=None, sorted_vocab=1, batch_words=MAX_WORDS_IN_BATCH, compute_loss=False, preserve_vocab = {}, callbacks=()):
        """
        ...
        super(Word2Vec_Ngram, self).__init__(
            ...
            fast_version=FAST_VERSION,word_gram=word_gram, context_gram=context_gram,preserve_vocab=preserve_vocab)
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        tally = 0
        if self.sg:
            tally += train_batch_sg_ngram(self, sentences, self.word_gram, self.context_gram, alpha, work, self.compute_loss, keep_overlap=self.keep_overlap)
        else:
            print("We are calling from cbow ngram w2vec")
            tally += train_batch_cbow_ngram(self, sentences, self.word_gram, self.context_gram, alpha, work, neu1, self.compute_loss , keep_overlap=self.keep_overlap)
    ...
    def train(self, sentences, total_examples=None, total_words=None,
        ...
        return super(Word2Vec_Ngram, self).train(
    ...
    def load(cls, *args, **kwargs):
        ...
        """
        try:
            return super(Word2Vec_Ngram, cls).load(*args, **kwargs)
...
class Word2VecVocab(utils.SaveLoad):
    def __init__(self, max_vocab_size=None, min_count=(5,5), sample=1e-3, sorted_vocab=True, null_word=0 , word_gram =(1,2), preserve_vocab={}):
        ...
        self.raw_vocab = {}
        self.preserve_vocab = preserve_vocab
        self.word_gram = word_gram
        self.word_gram_max = max(self.word_gram)
    ...
    def scan_vocab(self, sentences, progress_per=10000, trim_rule=None):
        ...
        for gram_iter in range(max(self.word_gram)):
            self.raw_vocab[gram_iter] = defaultdict(int)
        for sentence_no, sentence in enumerate(sentences):
            ...
            sentence_ngram = _compute_ngrams_words(sentence,self.word_gram_max)
            for sent_iter,sen_gram in enumerate(sentence_ngram):
                vocab_dummy = self.raw_vocab[sent_iter]
                for word_iter,word in enumerate(sen_gram):
                    vocab_dummy[word] += 1
                    vocab[word] += 1
                    vocab_dummy_index[word] = sent_iter
                    total_words += len(sen_gram)
                self.raw_vocab[sent_iter] = vocab_dummy
        ...
        ##### Cleaning raw vocab
        words_to_remove = [word_ for word_ in vocab_dummy if word_ not in vocab]
        for word_ in words_to_remove:
            del self.raw_vocab[vocab_dummy_index[word_]][word_]
        logger.info("removed %i from raw vocab", len(words_to_remove))
        logger.info(
    ...
    def prepare_vocab(self, hs, negative, wv, update=False, keep_raw_vocab=False, trim_rule=None,
        ...
        if not update:
            ...
            for word_iter in range(max(self.word_gram)):

                vocab_gram = self.raw_vocab[word_iter]
                for word, v in iteritems(vocab_gram):
                    if keep_vocab_item(word, v, min_count[word_iter], self.preserve_vocab, trim_rule=trim_rule):
                        retain_words.append(word)
                        retain_words_dict[word] = word_iter
                        retain_total += v
                        if not dry_run:
            ...
            logger.info(
                "min_count=%s retains %i unique words (%i%% of original %i, drops %i)",
                str(min_count), len(retain_words), retain_unique_pct, original_unique_total, drop_unique
            )
            original_total = retain_total + drop_total
            retain_pct = retain_total * 100 / max(original_total, 1)
            logger.info(
                "min_count=%s leaves %i word corpus (%i%% of original %i, drops %i)",
                str(min_count), retain_total, retain_pct, original_total, drop_total
            )
        else:
            ...
            for word_iter in range(max(self.word_gram)):
                vocab_gram = self.raw_vocab[word_iter]
                for word, v in iteritems(vocab_gram):
                    if keep_vocab_item(word, v, min_count, self.preserve_vocab, trim_rule=trim_rule):
                        if word in wv.vocab:
        ...
        for w in retain_words:
            index_to_check = retain_words_dict[w]
            v = self.raw_vocab[index_to_check][w]
