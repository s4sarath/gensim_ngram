...
try:
    from gensim.models._utils_any2vec import ft_hash as _ft_hash, compute_ngrams as _compute_ngrams , compute_ngrams_words as _compute_ngrams_words
    print("utils any2vec is a success")
except ImportError:
    print("Slower version")
    FAST_VERSION = -1
    ...
    def compute_individual_ngram(sentence_list, n_gram=2):
        ngram_words = []
        for iter_ in range(len(sentence_list)-(n_gram-1)):
            ngram_words.append(' '.join(sentence_list[iter_:(iter_+n_gram)]))
        return ngram_words

    def _compute_ngrams_words(sentence_list , max_n , min_n = 1):
        """Get the list of all possible ngrams for a given word.
        ...
        for n_gram in range(min_n,max_n+1):
            temp = []
            for iter_ in range(len(sentence_list)-(n_gram-1)):
                temp.append(' '.join(sentence_list[iter_:(iter_+n_gram)]))
            ngrams_words.append(temp)
        return ngrams_words
