...
from gensim.models.utils_any2vec import _compute_ngrams, _ft_hash , _compute_ngrams_words
...
try:
    from gensim.models.fasttext_inner_ngram import train_batch_sg_ngram, train_batch_cbow_ngram
    from gensim.models.fasttext_inner import FAST_VERSION, MAX_WORDS_IN_BATCH
    logger.info("Successfully loaded Cython versions of fasttext")
except ImportError:
    # failed... fall back to plain numpy (20-80x slower training than the above)
    logger.info("Failed to load Cython versions of fasttext")
    FAST_VERSION = -1
    MAX_WORDS_IN_BATCH = 10000

    def train_batch_cbow_ngram(model, sentences, alpha, word_gram , context_gram , keep_overlap = True, work=None, neu1=None):
        ...
        for sentence in sentences:
            sentence_vocab = [model.wv.vocab[w] for w in sentence if w in model.wv.vocab and
                                   model.wv.vocab[w].sample_int > model.random.rand() * 2 ** 32]
            n_gram_vocab_temp = {}
            for index_, sentence_ngram in enumerate(_compute_ngrams_words(sentence_vocab, max_n=max(max(word_gram),max(context_gram)))):
                n_gram_vocab_temp[index_] = sentence_ngram
            
            for word_iter in range(max(word_gram)):
                word_vocabs = n_gram_vocab_temp[word_iter]
                for pos, word in enumerate(word_vocabs):
                    reduced_window = np.random.randint(model.window)  # `b` in the original word2vec code
                    # now go over all words from the (reduced) window, predicting each one in turn            
                    for context_iter in range(max(context_gram)):
                        ...
                        for pos2, word2 in enumerate(current_window, start):
                            if pos2 == pos and word==word2:# don't train on the `word` itself
                                continue
                            if not keep_overlap and context_iter > 0:
                                if word in word2:
                                    if pos2 in range(pos-context_iter, pos+1):
                                        continue
                            word2_indices.append(word2.index)
    ...
    def train_batch_sg_ngram(model, sentences, word_gram , context_gram, alpha, work=None, neu1=None, keep_overlap = 1):
...
class FastText_Ngram(BaseWordEmbeddingsModel):
    ...
    """
    def __init__(self, sentences=None, word_gram = (1,2) , context_gram = (1,2) , keep_overlap = 1, sg=1, hs=0, size=100, alpha=0.025, window=5, min_count=(5,5),
                 max_vocab_size=None, word_ngrams=1, sample=1e-3, seed=1, workers=3, min_alpha=0.0001,
                 negative=5, cbow_mean=1, hashfxn=hash, iter=5, null_word=0, min_n=3, max_n=6, sorted_vocab=1,
                 bucket=2000000, trim_rule=None, batch_words=MAX_WORDS_IN_BATCH, preserve_vocab = {}, callbacks=()):
        ...
        word_gram: The tuple of ngrams to be considered for words. eg: (1,2) considers both unigram and bigram,
            (1,3) considers unigram,bigram and trigram
        context_gram: The tuple of ngrams to be considered for words. eg: (1,2) considers both unigram and bigram,
            (1,3) considers unigram,bigram and trigram
        keep_overlap: 1. if 1 consider overlapping of word, else not. eg: "i like to be there". if centre_word = 'like to',
            if keep_overlap = 1, all context will be considered, else "i like" and "to be" will be avoided in case of a bigram
        preserve_vocab: a dict, the words in preserve vocab will be preserved irrespective of their min_count
        min_count: a tuple of minimum occurence of words to be preserved, (3,5,7) means, for unigram min_count is 3, for bigram
            min_count is 5, for trigram min_count is 7.
        ...
        """
        assert max(word_gram) == len(min_count) #### check min_count has been specified for all ngrams
        self.load = call_on_class_only
        self.load_fasttext_format = call_on_class_only
        self.callbacks = callbacks
        self.word_ngrams = int(word_ngrams)
        self.word_gram   = word_gram
        self.context_gram = context_gram
        ...
        self.vocabulary = FastTextVocab(
            ...
            sorted_vocab=bool(sorted_vocab), null_word=null_word, word_gram = word_gram, preserve_vocab=preserve_vocab)
        self.trainables = FastTextTrainables(
            vector_size=size, seed=seed, bucket=bucket, hashfxn=hashfxn)
        self.wv.bucket = self.bucket

        super(FastText_Ngram, self).__init__(
            ...
            hs=hs, negative=negative, cbow_mean=cbow_mean, min_alpha=min_alpha, fast_version=FAST_VERSION,word_gram=word_gram, context_gram=context_gram,preserve_vocab=preserve_vocab)
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        tally = 0
        if self.sg:
            tally += train_batch_sg_ngram(self, sentences, self.word_gram, self.context_gram, alpha, work, neu1 , keep_overlap=self.keep_overlap)
        else:
            tally += train_batch_cbow_ngram(self, sentences, self.word_gram, self.context_gram, alpha, work, neu1, keep_overlap=self.keep_overlap)
    ...
    def train(self, sentences, total_examples=None, total_words=None,
        ...
        super(FastText_Ngram, self).train(
    ...
    def save(self, *args, **kwargs):
        ...
        super(FastText_Ngram, self).save(*args, **kwargs)
    ...
    def load(cls, *args, **kwargs):
        ...
        """
        try:
            model = super(FastText_Ngram, cls).load(*args, **kwargs)
...
class FastTextVocab(Word2VecVocab):
    def __init__(self, max_vocab_size=None, min_count=5, sample=1e-3, sorted_vocab=True, null_word=0 , word_gram=(1,2), preserve_vocab={}):
        super(FastTextVocab, self).__init__(
            max_vocab_size=max_vocab_size, min_count=min_count, sample=sample,
            sorted_vocab=sorted_vocab, null_word=null_word, word_gram=word_gram, preserve_vocab=preserve_vocab)
