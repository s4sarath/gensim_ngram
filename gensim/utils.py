...
def prune_vocab(vocab, min_reduce, trim_rule=None):
    ...
    for w in list(vocab):  # make a copy of dict's keys
        if not keep_vocab_item(w, vocab[w], min_reduce, preserve_vocab, trim_rule):  # vocab[w] <= min_reduce:
            result += vocab[w]
            del vocab[w]
    logger.info(
...
def prune_vocab_return(vocab, min_reduce, trim_rule=None):
    """Remove all entries from the `vocab` dictionary with count smaller than `min_reduce`.

    Modifies `vocab` in place, returns the vocab.
    Parameters
    ----------
    vocab : dict
        Input dictionary.
...
def keep_vocab_item(word, count, min_count, preserve_vocab={}, trim_rule=None ):
    ...
    preserve_vocab : dictionary
        Dictionary of words {word: 1.0}, value of the dictionary can be anything, but word should
        be the key. Using dictionary for faster searching, compared to list. If "word" in preserve_vocab
        we return a bool True.
    trim_rule : function, optional
        Function for trimming entities from vocab, default behaviour is `vocab[w] <= min_reduce`.
